...
// Function para ocultar  pantalla principal,
// mostar los usuarios.
...
function checkContraseña (inputContraseña) {
    ...
    for (let i = 0; i < cuentas.length; i++){
        if (usuarioActual == cuentas[i].nombre && contraseñaFinal == cuentas[i].password) {
            ...
            const resultado = document.createTextNode (`password error try again`);
...
function checkSaldo (saldoIngresado) {
    if ((saldoIngresado + saldoActual) > 990){
        const mensajeError = document.createTextNode ("Amount not allowed. Enter alower value.")
...
function mostrarTransacción (saldoIngresado, saldoActual) {
    ...
    const tituloRetiro = document.createTextNode (`Deposited:`);
...
function checkSaldoRetirar (saldoIngresado) {
    ...
    if ((saldoActual - saldoIngresado) < 10){
        const mensajeError = document.createTextNode ("you must leave a minimum of 10$ in the account. Enter lower value.")
